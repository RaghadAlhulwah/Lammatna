...
function initializeSampleData() {
  ...
  if (users.length === 0) {
    ...
    saveJSON(STORAGE.USERS, sampleUsers);
  }
  ...
  if (gatherings.length === 0) {
    const sampleGatherings = [
      {
        id: "sample1",
        name: "حفلة تخرج الجامعة",
        category: "حفلة تخرج",
        date: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
        location: "مطعم النخيل، الرياض",
        createdAt: nowISO(),
        createdBy: "ahmed@example.com",
        code: "GRAD2024",
        shareableLink: `${window.location.origin}/gatherings.html?joincode=GRAD2024`,
        participants: ["ahmed@example.com", "fatima@example.com", "mohammed@example.com"],
        tasks: [
          {
            id: "task1",
            title: "شراء الكعك",
            notes: "كعك شوكولاتة كبير",
            type: "item",
            assignedTo: "fatima@example.com",
            status: "pending",
            createdAt: nowISO()
          },
          {
            id: "task2", 
            title: "حجز المطعم",
            notes: "التأكد من توفر 30 كرسي",
            type: "task",
            assignedTo: "ahmed@example.com",
            status: "done",
            createdAt: nowISO()
          }
        ],
    ...
    saveJSON(STORAGE.GATHERINGS, sampleGatherings);
  }
}
...
function addSampleUserLogin() {
  ...
  if (document.querySelector('.sample-users')) return;
  
  const sampleUsersContainer = document.createElement('div');
  sampleUsersContainer.className = 'sample-users';
  sampleUsersContainer.style.marginTop = '20px';
  sampleUsersContainer.style.padding = '15px';
  sampleUsersContainer.style.background = '#FFF8DC';
  sampleUsersContainer.style.borderRadius = '10px';
  sampleUsersContainer.style.border = '2px dashed #F4A460';
  
  sampleUsersContainer.innerHTML = `
    <h4 style="margin-bottom: 10px; color: #8B4513; text-align: center;">تجربة التطبيق:</h4>
    <p style="margin-bottom: 10px; color: #7a5c3c; font-size: 0.9rem; text-align: center;">
      يمكنك الدخول باستخدام أحد الحسابات الوهمية:
    </p>
    <div style="display: flex; flex-direction: column; gap: 8px;">
      <button type="button" class="sample-user-btn" data-user="ahmed@example.com" style="background: white; border: 2px solid #8B4513; color: #8B4513; padding: 10px; border-radius: 8px; cursor: pointer; text-align: right; display: flex; align-items: center; gap: 8px;">
        <i class="fas fa-user"></i> أحمد (منشئ فعاليات)
      </button>
      <button type="button" class="sample-user-btn" data-user="fatima@example.com" style="background: white; border: 2px solid #8B4513; color: #8B4513; padding: 10px; border-radius: 8px; cursor: pointer; text-align: right; display: flex; align-items: center; gap: 8px;">
        <i class="fas fa-user"></i> فاطمة (مشاركة في فعاليات) 
      </button>
      <button type="button" class="sample-user-btn" data-user="mohammed@example.com" style="background: white; border: 2px solid #8B4513; color: #8B4513; padding: 10px; border-radius: 8px; cursor: pointer; text-align: right; display: flex; align-items: center; gap: 8px;">
        <i class="fas fa-user"></i> محمد (مستخدم عادي)
      </button>
    </div>
    <p style="margin-top: 10px; color: #7a5c3c; font-size: 0.8rem; text-align: center;">
      كلمة المرور لجميع الحسابات: <strong>password</strong>
    </p>
  `;
  
  loginForm.parentNode.insertBefore(sampleUsersContainer, loginForm.nextSibling);
  
  sampleUsersContainer.querySelectorAll('.sample-user-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      const email = this.dataset.user;
      document.getElementById('l-email').value = email;
      document.getElementById('l-password').value = 'password';
      showNotif(`تم تعبئة بيانات ${email} - كلمة المرور: password`);
    });
